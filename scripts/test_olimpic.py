...
parser = argparse.ArgumentParser(description='OMR evaluation (SER) on the OLiMPiC test set')
...
scanned_data_pairs = [x for x in dataset.test_data_pairs['olimpic'] if 'scanned' in str(x['lmx'])]
synthetic_data_pairs = [x for x in dataset.test_data_pairs['olimpic'] if 'synthetic' in str(x['lmx'])]

dataset.test_data_pairs = {'olimpic-scanned': scanned_data_pairs, 'olimpic-synthetic': synthetic_data_pairs}

olimpic_loader = dataset.get_specific_testset_loader('olimpic-scanned', 'pt', 'lmx', batch_size=32)
...
olimpic_loader = dataset.get_specific_testset_loader('olimpic-synthetic', 'pt', 'lmx', batch_size=32)
...
with torch.inference_mode():
  for batch in tqdm(olimpic_loader):
    in_modal, in_mask, target_in, target_out, modal_idx, token_heights, in_pos, target_in_pos = batch['in_modal'], batch['in_mask'], batch['target_in'], batch['target_out'], batch['modal_idx'], batch['token_height'], batch['in_pos'], batch['target_in_pos']  
    inf_out = model.inference(in_modal, in_pos, modal_idx, in_mask=in_mask, sampling_method='argmax', temperature=0.1)
    total_inf_out.append(inf_out.cpu())
    total_target_out.append(target_out.cpu())
gold_lmx, pred_lmx = [], []
for inf_out, target_out in zip(total_inf_out, total_target_out):
