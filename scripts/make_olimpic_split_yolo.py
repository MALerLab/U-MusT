...
dataset_dir = Path('dataset/olimpic_yolo')  # root of the YOLO-cropped OLiMPiC dataset
...
for pt_fn in tqdm(pt_fns):
  target_lmx_fn = pt_fn.parent.parent.parent.parent / (pt_fn.stem.replace('-crop_resize', '').replace('_yolo_resized', '') + '.lmx')
...
with open(output_dir / 'olimpic-yolo.json', 'w') as f:
