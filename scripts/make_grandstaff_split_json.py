...
dataset_dir = Path('dataset/grandstaff-lmx')  # root of the GrandStaff-LMX dataset
...
for pt_fn in tqdm(pt_fns):
  target_lmx_fn = pt_fn.parent.parent.parent.parent / (pt_fn.stem.replace('_distorted', '') + '.lmx')
...
for split_name, split_txt_name in split_txt_names.items():
  ...
  txt_fn = dataset_dir / split_txt_name
  print(txt_fn)
...
train_pairs = [x for x in pairs if dict_by_piece[x['lmx'].split('.')[0]] == 'train']
valid_pairs = [x for x in pairs if dict_by_piece[x['lmx'].split('.')[0]] == 'valid']
test_pairs = [x for x in pairs if dict_by_piece[x['lmx'].split('.')[0]] == 'test']
...
with open(output_dir / 'grandstaff-lmx.json', 'w') as f:
