...
from .note_event_dataclasses import Note, NoteEvent
from .note2event import validate_notes, trim_overlapping_notes
from .note2event import note2note_event
""" midi2note:
        ...
        if msg.type == 'program_change' and msg.channel not in reserved_channels:
            ...
            if program_state[msg.channel] == None:
                if force_all_program_to == None:
                    # print(f'📕 midi2note: program_change message is missing. Use `force_all_program_to` option')
                    raise ValueError(
