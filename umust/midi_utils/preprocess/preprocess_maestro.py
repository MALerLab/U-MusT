...
import numpy as np
from umust.midi_utils.audio import get_audio_file_info
from umust.midi_utils.midi import midi2note, note_event2midi
from umust.midi_utils.note2event import note2note_event, note_event2event
from umust.midi_utils.event2note import event2note_event
from umust.midi_utils.note_event_dataclasses import Note, NoteEvent
from umust.midi_utils.utils import note_event2token2note_event_sanity_check, assert_note_events_almost_equal
...
def preprocess_maestro16k(data_home=os.PathLike,
    ...
    base_dir = os.path.join(data_home, dataset_name)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--data_home', type=str, default='dataset/')
  parser.add_argument('--dataset_name', type=str, default='maestro-v3.0.0/')
  parser.add_argument('--ignore_pedal', type=bool, default=False)
  parser.add_argument('--sanity_check', type=bool, default=False)
  args = parser.parse_args()
  preprocess_maestro16k(data_home=args.data_home, dataset_name=args.dataset_name, ignore_pedal=args.ignore_pedal, sanity_check=args.sanity_check)
