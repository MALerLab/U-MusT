...
import numpy as np
from .note_event_dataclasses import Note, NoteEvent, NoteEventListsBundle, Event
...
def note_event2event(note_events: List[NoteEvent],
    ...
    if end_time is not None:
        end_tick = round(end_time * tps)
        if end_tick > tick_state:
            shift_ticks = end_tick - start_tick
            events.append(Event(type='shift', value=shift_ticks))
    return events
