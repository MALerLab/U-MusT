...
secret named `HF_TOKEN` whose owner has been granted access; the app downloads the piano run, both codecs and the
YOLO detectors on first start. A GPU (T4 or better, ~6 GB) is required; ZeroGPU is supported through the optional
`spaces` package.
