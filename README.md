...
- `omr_direction_all` now specifies the `unirqvae3` tokenizer, matching the shipped manifests. The paper's strings model was trained with the earlier `unirqvae` generation, so retraining from this recipe produces a `unirqvae3`-generation model rather than a reproduction of that checkpoint.
